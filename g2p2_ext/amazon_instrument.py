...
def get_data(file, meta_file, dataset):
    ...
    # wanted_df = df[['asin', 'title', 'description', 'category']]
    ...
    print('-----descrip[92312]', descrip[92312])
    ...
    print('-------process_1[92312]', process_1[92312])
    ...
    # print('--------process_2[1141]', process_2[1141])
    ...
    for i in range(len(reviewerID)):
        review_dict[reviewerID[i]].append(review_text[i])

    id_text = {}
    for i in review_dict:
        id_text[id_map[i]] = ' '.join(review_dict[i])
    ...
    fea_m = []
    # print('id_fea_dict[8]', id_fea_dict[8])
    print('id_fea_dict[7]', id_fea_dict[7])
    print('id_fea_dict[5]', id_fea_dict[5])
    sorted_id_f_dict = collections.OrderedDict(sorted(id_fea_dict.items()))
    print('sorted_id_f_dict[5]', sorted_id_f_dict[5])
    for i in edge_node:
    ...
    json.dump(id_text, open('./tmp/{}_id_labels.json'.format(dataset), 'w'))
