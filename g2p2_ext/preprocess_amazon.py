...
def split_user_bytime(qualified_review_df, train_size=0.7):
    ...
    # 根据用户的最后一次评论时间排序
    user_last_review_time = qualified_review_df.groupby(
        'reviewerID')['realWorldTime'].max().sort_values()
    ...
    # 根据用户ID分割数据集
    train_df = qualified_review_df[qualified_review_df['reviewerID'].isin(
        train_users)]
    test_df = qualified_review_df[qualified_review_df['reviewerID'].isin(
        test_users)]
...
def get_data(file, meta_file, dataset):
    ...
    val_id_map = {id_key: index + start_index for index,
                  id_key in enumerate(val_ids) if id_key not in train_id_map}
    rv_val_id_map = {index + start_index: id_key for index,
                     id_key in enumerate(val_ids) if id_key not in train_id_map}
    ...
    test_id_map = {id_key: index + start_index for index, id_key in enumerate(
        test_ids) if id_key not in train_id_map and id_key not in val_id_map}
    rv_test_id_map = {index + start_index: id_key for index, id_key in enumerate(
        test_ids) if id_key not in train_id_map and id_key not in val_id_map}
    ...
    for id, df in enumerate([train_df, val_df, test_df]):
        ...
        if id == 0:
            ...
            for u_id, items in qualified_user_dict.items():
                ...
                supp_indices = np.random.choice(
                    range(len(items)), 10, replace=False)
                supp_items = [items[i] for i in supp_indices]
                supp_ratings = [qualified_user_rating[u_id][i]
                                for i in supp_indices]
                supp_ts = [qualified_user_ts[u_id][i] for i in supp_indices]
                ...
                remaining_ratings = [qualified_user_rating[u_id][i]
                                     for i in remaining_indices]
                remaining_ts = [qualified_user_ts[u_id][i]
                                for i in remaining_indices]
                ...
                supp_out_edges += [str(u_id)+'\t'+str(i_id) +
                                   f'\t{r:.0f}'+f'\t{t:.0f}' for i_id, r, t in zip(supp_items, supp_ratings,  supp_ts)]

                test_out_edges += [str(u_id)+'\t'+str(i_id)+f'\t{r:.0f}'+f'\t{t:.0f}' for i_id, r, t in zip(
        ...
        if id == 0:
            ...
            id_text = {all_id_map[key]: ' '.join(
                texts) for key, texts in user_review_dict.items()}
            ...
            json.dump(id_text, open(
                './tmp/{}_{}_text.json'.format(dataset, "train"), 'w'))
            ...
            json.dump(id_text, open(
                './tmp/{}_{}_text.json'.format(dataset, "val"), 'w'))
            ...
            supp_id_text = {all_id_map[key]: ' '.join(
                texts) for key, texts in supp_user_review_dict.items()}

            test_id_text = {all_id_map[key]: ' '.join(
                texts) for key, texts in test_user_review_dict.items()}
            ...
            json.dump(supp_id_text, open(
                './tmp/{}_support_text.json'.format(dataset), 'w'))
            json.dump(test_id_text, open(
                './tmp/{}_test_text.json'.format(dataset), 'w'))
