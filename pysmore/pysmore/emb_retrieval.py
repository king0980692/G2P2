...
def process_ui_query3(uid):
    ...
    for u in uid:
        ...
        pool_vec = np.array([np.array(embed[it]) if it in embed and it not in observed_items else np.zeros((embed_dim,)) for it in item_pool])
    ...
    for id, u in enumerate(uid):
        top_k_list = np.array(item_pool)[np.argsort(scores[:,id])[::-1]].tolist()[:len(test_ui[u])]
...
def process_ii_query3(uid):
    ...
    pool_array = []
    for u in uid:
        q_vec = np.sum([embed[i] if i in embed else np.zeros((embed_dim,)) for i in train_ui[u]], axis=0)
...
def process_args():
    ...
    parser.add_argument('--query_emb', help='query embeddding file')
    parser.add_argument('--candidate_emb', help='candidate embeddding file')

    parser.add_argument('--embed_dim', type=int, default=64, help='emedding demensions')
    ...
    parser.add_argument('--faiss', action='store_true', help='use faiss to compute similarities')
    parser.add_argument('--annoy', action='store_true', help='use annoy to compute similarities')
...
def recommendations():
    ...
    if args.annoy:
        ...
        for uid in tqdm(queries[:args.num_test]):
            ...
            ui_results = [uid, str(len(test_ui[uid]))] # uid, total_len
            top_k_list = [rv_iid_map[r] for r in rec if r not in observed_items][:len(test_ui[uid])]
        ...
        for uid in tqdm(queries[:args.num_test]):
            _, rec = faiss_index.search(
                        np.expand_dims(np.array(embed[uid]),axis=0).astype(np.float32),
                        1000 
                    )
            ...
            ui_results = [uid, str(len(test_ui[uid]))] # uid, total_len
            top_k_list = [rv_iid_map[r] for r in rec[0] if r not in observed_items][:len(test_ui[uid])]
        ...
        with mp.get_context('spawn').Pool(processes=args.worker, initializer=initializer, initargs=((sim,iids,cold_iids,train_ui,test_ui,embed,args.embed_dim),)) as pool:
            ...
            rec_ui = list(tqdm(pool.imap(process_ui_query, queries), total=len(queries) )) 
        ...
        with mp.get_context('spawn').Pool(processes=args.worker, initializer=initializer, initargs=((sim,iids,cold_iids,train_ui,test_ui,embed,args.embed_dim),)) as pool:
            ...
            rec_ii = list(tqdm(pool.imap(process_ii_query2, queries), total=len(queries) )) 
