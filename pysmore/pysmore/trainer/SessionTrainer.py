...
class ParallelSessRecTrainer():
    def __init__(self,
        ...
        self.early_stopper = EarlyStopper(self.train_args.es_patience,
                                          self.early_stop_by)
    ...
    def _train_epoch(self,
        ...
        for id, (input, targets, finishedMask, validMask) in pbar:
            ...
            predictions += preds.argmax(1).tolist() if len(
                preds.shape) > 1 else preds.tolist()
        ...
        if self.writer:
            self.writer.add_scalar(
                'Loss/Train', loss.item(), epoch_i*total_len+id)
        ...
        for epoch_i in range(self.train_args.n_train_epochs):
            ...
            if self.eval_dataset is not None:
                ...
                epoch_valid_loss, metric_dict, x = evaluate(
                    self.metric_fns, self.model, self.device, self.loss_cls,
                    self.eval_dataset, epoch_i, self.writer, debug=False)
                ...
                if not self.train_args.silence:
                    ...
                    print(
                        "\n[ \033[92mTrain Loss: {:8.6f}\033[0m /".format(
                            epoch_train_loss), end="")

                    # print the val loss with green color
                    print(" \033[91mValid Loss: {:8.6f}\033[0m ]".format(
                        epoch_valid_loss))
                ...
                if list(metric_dict.values())[0] < best_score:
                    ...
                    if not self.train_args.silence:
                        print("Save best model into {}".format(
                ...
                # early stopping
                es_by = epoch_valid_loss if self.early_stop_by == "loss" \
